...
namespace NS_TEAM_PROJECT
{
	void genRandNormalDist(std::vector<int>& myNums, double mean, double stdDev)
	{
		...
		if (mean == 0 || stdDev == 0)
		{
			std::cout << "Mean or Standard Deviation equal 0. Distribution not possible." << std::endl << std::endl; // message informing users of reason for no generation
			return; // return to orignal function
		}

		const int size = 20000; // 20000 random integers specified in the project description
		...
		if (mean < 10) // if the mean is lesser than 10 the size is set to the mean plus 6 to make it a range of 10
		{
			myNums.resize((int)mean + 6, 0); // resizes the vector to organize its bins for future printing using the mean as it is the median 
											 // and adds 6 to make the range large enough to output 10 bins
		}
		else // if the mean is greater than or equal to 10 the size is set to the mean plus 11 to make it a range of 20
		{
			myNums.resize((int)mean + 11, 0); // resizes the vector to organize its bins for future printing using the mean as it is the median 
										 	  // and adds 11 to make the range large enough to output 20 bins
		}
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (temp >= 0 && temp < myNums.size()) // checks random number is within 0 and the max size of the vector
			{
				myNums[temp]++; // for each corresponding value their frequency is increased by treating the index as the value and its held element as frequency
			}
		}
	}
